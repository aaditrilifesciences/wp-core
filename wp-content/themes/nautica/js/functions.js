...
( function( $ ) {
	...
	( function() {
		...
		button.on( 'click.engotheme', function() {
	 ...
	 */
	_window.on( 'hashchange.engotheme', function() {
	...
	$( function() {
		// Search toggle.
		$( '.search-toggle' ).on( 'click.engotheme', function( event ) {
			var that    = $( this ),
...
**********************************************************************/
    ...
    jQuery(document).ready( function( $ ){
        ...
        $('form.login-form').submit( function(){
                ...
                $('.alert', this).remove(); 
                $.ajax({
                  url: nauticaAjax.ajaxurl,
                  type:'POST',
                  dataType: 'json',
                  data:  $(this).serialize()+"&action=engoajaxlogin"
                }).done(function(data) {
                    if( data.loggedin ){
        ...
        $('form.lostpassword-form').submit( function(){
                ...
                $.ajax({
                  ...
                  type:'POST',
                  dataType: 'json',
                  data:  $(this).serialize()+"&action=engoajaxlostpass"
                }).done(function(data) {
                    if( data.loggedin ){
                        $this.prepend( '<div class="alert alert-info">'+data.message+'</div>' );
                        location.reload(); 
	...
	$( document ).ready( function(){
        ...
        var $animation_engoPullDown = $('.animation-element.engo-PullDown');
        var $animation_tinDownIn = $('.animation-element.tin-DownIn');
        var $animation_hover_in = $('.animation-hover-element.fade-in');
        var $window = $(window);
        ...
        function check_if_in_view() {
            ...
            $.each($animation_hover_in, function() {
                var $element = $(this);
                    $element.hover(
                        function(){
            ...
            $.each($animation_tinDownIn, function() {
                var $element = $(this);
            ...
            $.each($animation_engoPullDown, function() {
                var $element = $(this);
                var element_top_position = $element.offset().top;
                var element_bottom_position = (element_top_position + 100);
         ...
         */
        if( $('.page-static-left') ){
            $('.page-static-left .button-action').click( function(){
