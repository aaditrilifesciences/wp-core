...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'valey' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'valey' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'valey' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      echo '<h1><img src="' . FX_VALEY_URL . '/assets/images/logo-white.png' . '" /></h1>';
      echo '<fieldset>';
      echo ( $this->settings['ajax_save'] === true ) ? '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'valey' ) .'</span>' : '';
      submit_button( esc_html__( 'Save', 'valey' ), 'primary', 'save', false, array( 'data-ajax' => $this->settings['ajax_save'], 'data-save' => esc_html__( 'Saving...', 'valey' ) ) );
      submit_button( esc_html__( 'Restore', 'valey' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );
      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'valey' ) .'</a>' : '';
      ...
      echo 'Valey made with love <strong>by 516x Team</strong>';
